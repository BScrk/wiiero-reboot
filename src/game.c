...
void wiiero_init(game_t *g)
{
  ...
  // Give window time to appear on screen
  SDL_Delay(100);
  SDL_PumpEvents();
...
void wiiero_update_world(game_t *g)
{
  ...
  player_event_update(g->worms[PLAYER_1], g->wiiero_map, g->worms[PLAYER_3]);
  player_event_update(g->worms[PLAYER_1], g->wiiero_map, g->worms[PLAYER_4]);

  player_event_update(g->worms[PLAYER_2], g->wiiero_map, g->worms[PLAYER_1]);
  player_event_update(g->worms[PLAYER_2], g->wiiero_map, g->worms[PLAYER_3]);
  player_event_update(g->worms[PLAYER_2], g->wiiero_map, g->worms[PLAYER_4]);

  player_event_update(g->worms[PLAYER_3], g->wiiero_map, g->worms[PLAYER_1]);
  player_event_update(g->worms[PLAYER_3], g->wiiero_map, g->worms[PLAYER_2]);
  player_event_update(g->worms[PLAYER_3], g->wiiero_map, g->worms[PLAYER_4]);

  player_event_update(g->worms[PLAYER_4], g->wiiero_map, g->worms[PLAYER_1]);
  player_event_update(g->worms[PLAYER_4], g->wiiero_map, g->worms[PLAYER_2]);
  player_event_update(g->worms[PLAYER_4], g->wiiero_map, g->worms[PLAYER_3]);
  ...
  player_is_aiming(g->worms[PLAYER_1], g->worms[PLAYER_3]);
  player_is_aiming(g->worms[PLAYER_1], g->worms[PLAYER_4]);

  player_is_aiming(g->worms[PLAYER_2], g->worms[PLAYER_1]);
  player_is_aiming(g->worms[PLAYER_2], g->worms[PLAYER_3]);
  player_is_aiming(g->worms[PLAYER_2], g->worms[PLAYER_4]);
  
  player_is_aiming(g->worms[PLAYER_3], g->worms[PLAYER_1]);
  player_is_aiming(g->worms[PLAYER_3], g->worms[PLAYER_2]);
  player_is_aiming(g->worms[PLAYER_3], g->worms[PLAYER_4]);
  
  player_is_aiming(g->worms[PLAYER_4], g->worms[PLAYER_1]);
  player_is_aiming(g->worms[PLAYER_4], g->worms[PLAYER_2]);
  player_is_aiming(g->worms[PLAYER_4], g->worms[PLAYER_3]);
...
static __inline__ int wiiero_player_warning(player_id player, game_t *g)
{
  ...
  case GAME_CAPTURE_FLAG_MODE:
    //FIXME
    /*switch(player){
      case PLAYER_1:return g->worms[PLAYER_2]->worms_status & STATUS_HAVE_FLAG;
      case PLAYER_2:return g->worms[PLAYER_2]->worms_status & STATUS_HAVE_FLAG;
      case PLAYER_3:return g->worms[PLAYER_2]->worms_status & STATUS_HAVE_FLAG;
      default:      return g->worms[PLAYER_2]->worms_status & STATUS_HAVE_FLAG;
    }*/
...
void wiiero_blit_world(game_t *g)
{
  ...
  for(p = PLAYER_1; p < NB_PLAYERS; p++){
    if(p != PLAYER_1)
      player_show_on_cam(g->worms[p], g->wiiero_cameras[PLAYER_2_GAME_ZONE_CAM], wiiero_player_warning(PLAYER_1, g));
    if(p != PLAYER_2)    
      player_show_on_cam(g->worms[p], g->wiiero_cameras[PLAYER_2_GAME_ZONE_CAM], wiiero_player_warning(PLAYER_2, g));
    if(p != PLAYER_3)
      player_show_on_cam(g->worms[p], g->wiiero_cameras[PLAYER_2_GAME_ZONE_CAM], wiiero_player_warning(PLAYER_3, g));
    if(p != PLAYER_4)
      player_show_on_cam(g->worms[p], g->wiiero_cameras[PLAYER_2_GAME_ZONE_CAM], wiiero_player_warning(PLAYER_4, g));
  }

  DBG(" - BLIT PLAY\n");
  for(p = PLAYER_1; p < NB_PLAYERS; p++)
    player_show(g->worms[p], wiiero_player_warning(PLAYER_1, g));
  ...
  if (g->wiiero_game_status == GAME_PLAYING)
  {
    ...
    blit_bullets( g->wiiero_bullets
                ...
                , g->wiiero_cameras[PLAYER_3_GAME_ZONE_CAM]
                , g->wiiero_cameras[PLAYER_4_GAME_ZONE_CAM]              
              );
  ...
  if (g->wiiero_opt_mini_map)
    update_minimap( g->wiiero_map
                  ...
                  , g->worms[PLAYER_3]->worms.pos_x
                  , g->worms[PLAYER_3]->worms.pos_y
                  , g->worms[PLAYER_4]->worms.pos_x
                  , g->worms[PLAYER_4]->worms.pos_y                                    
...
static __inline__ void wiiero_restart_game(game_t *g)
{
  ...
  if (first != 0)
  {
    ...
    for(int8_t p = PLAYER_1; p < NB_PLAYERS; p++){
      g->worms[p]->worms_status = DEFAULT_PLAYER_STATUS;
      g->worms[p]->worms_action = DEFAULT_PLAYER_ACTIONS;
      player_new_position(g->worms[p], g->wiiero_map->layers[GROUND_MAP_LAYER], g->wiiero_map->layers[STATICS_MAP_LAYER]);
      player_reset(g->worms[p], g->wiiero_map->layers[GROUND_MAP_LAYER], g->wiiero_map->layers[STATICS_MAP_LAYER]);
...
static __inline__ void wiiero_show_weapons(game_t *g, player_id id)
{
  ...
  font_print_strict_pos(g->wiiero_cameras[camera_id], wiiero_label[WIIERO_LANG_WSELECT_LABEL], g->wiiero_cameras[camera_id]->w / 10, g->wiiero_cameras[camera_id]->h / 4, FONT_STANDARD);
  for (i = 0; i < WEAPON_SLOTS; i++)
  {
    font_print_strict_pos(g->wiiero_cameras[camera_id], weapon_get_name(g->worms[id]->weapon_slots[i]->id), g->wiiero_cameras[camera_id]->w / 10 + 10, g->wiiero_cameras[camera_id]->h / 4 + (i + 2) * 10, (g->worms[id]->selected_weapon == i) ? FONT_SELECTED : FONT_STANDARD);
  }
  ...
  if (g->worms[id]->worms_action & ACTION_CANCEL)
  {
    for (i = 0; i < WEAPON_SLOTS; i++)
    {
      ...
      if (g->wiiero_opt_xtra_weap)
        g->worms[id]->weapon_slots[i] = weapon_init(rand() % MAX_WEAPONS_AND_XTRA);
      else
        g->worms[id]->weapon_slots[i] = weapon_init(rand() % MAX_WEAPONS);
    }
  }

  font_print_strict_pos(g->wiiero_cameras[camera_id], wiiero_label[WIIERO_LANG_WSELECT_KO], g->wiiero_cameras[camera_id]->w / 10, 3 * g->wiiero_cameras[camera_id]->h / 4, FONT_STANDARD);
  font_print_strict_pos(g->wiiero_cameras[camera_id], wiiero_label[WIIERO_LANG_WSELECT_OK], g->wiiero_cameras[camera_id]->w / 10, 3 * g->wiiero_cameras[camera_id]->h / 4 + (font_get_height(FONT_STANDARD) + 2), FONT_STANDARD);
} /*--------------------------------------------------------------------------*/
/* Weapon selection */
static __inline__ void wiiero_wselect(game_t *g)
{
  ...
  if ((selection_done[PLAYER_1]) && (selection_done[PLAYER_2]))
  {
    /*FINISH*/
...
static __inline__ void wiiero_menu(game_t *g)
{
  ...
  for (i = 0; i < MENU_MAX; i++)
  {
    if (i != selected_menu_entry)
      font_print_center(g->wiiero_cameras[PLAYER_1_GAME_ZONE_CAM], wiiero_menu[i].label, g->wiiero_cameras[PLAYER_1_GAME_ZONE_CAM]->w / 2, g->wiiero_cameras[PLAYER_1_GAME_ZONE_CAM]->h / 6 + (i * 15), FONT_STANDARD);
    else
  ...
  if (g->wiiero_game_status == GAME_MENU)
  {
    ...
    if (g->worms[PLAYER_1]->worms_action & ACTION_CANCEL || g->worms[PLAYER_2]->worms_action & ACTION_CANCEL)
      selected_menu_entry = MENU_RETURN;

    if (g->worms[PLAYER_1]->worms_action & ACTION_OK || g->worms[PLAYER_2]->worms_action & ACTION_OK)
    {
      ...
      else if (!round_running && wiiero_menu[selected_menu_entry].id == MENU_RETURN)
...
char *option_format_gamemode_cb(char *label, void *data)
{
  ...
  {
  case GAME_DEATHMATCH_MODE:
    snprintf(string, 127, "%s %s", label, wiiero_label[WIIERO_LANG_DMA]);
    break;
  case GAME_OF_TAG_MODE:
...
{
  static char string[128];
  snprintf(string, 127, "%s %.2dm%.2ds", label, (*((Uint16 *)data)) / 60, (*((Uint16 *)data)) % 60);
  return string;
} /*--------------------------------------------------------------------------*/
...
{
  static char string[128];
  snprintf(string, 127, "%s %s", label, (*((Uint8 *)data) == 0) ? wiiero_label[WIIERO_LANG_INFO_OFF] : wiiero_label[WIIERO_LANG_INFO_ON]);
  return string;
} /*--------------------------------------------------------------------------*/
...
{
  static char string[128];
  snprintf(string, 127, "%s %s", label, (get_nb_loaded_lang_files() == 0) ? "standard" : wiiero_lang_files[*((Uint8 *)data)]);
  return string;
} /*--------------------------------------------------------------------------*/
...
{
  /* Décrémenter la résolution (avec wrap-around) */
  screen_res_t *res = (screen_res_t *)val;
  if (*res == RES_320_240)
  {
    *res = MAX_RES - 1; /* Retour à la dernière résolution */
  }
  else
  {
    (*res)--;
...
{
  /* Incrémenter la résolution (avec wrap-around) */
  screen_res_t *res = (screen_res_t *)val;
  (*res)++;
  if (*res >= MAX_RES)
  {
    *res = RES_320_240; /* Retour à la première résolution */
  }
}
...
{
  static char string[128];
  snprintf(string, 127, "%s %dx%d", label, screen_get_width(*((screen_res_t *)data)), screen_get_height(*((screen_res_t *)data)));
  return string;
} /* ------------------------------------------------------------------------- */

/* ------------------------- Option Action Handler -------------------------- */
static __inline__ void wiiero_option_actions(game_t *g, int *selected_option_entry, const int OPTION_RETURN, const int OPTION_MAX, const option_enty_t *wiiero_option)
{
  if (g->worms[PLAYER_1]->worms_action & ACTION_UP || g->worms[PLAYER_2]->worms_action & ACTION_UP)
  {
      *selected_option_entry = (*selected_option_entry == 0) ? OPTION_MAX - 1 : (*selected_option_entry - 1);
  } else if (g->worms[PLAYER_1]->worms_action & ACTION_DOWN || g->worms[PLAYER_2]->worms_action & ACTION_DOWN) {
    *selected_option_entry = (*selected_option_entry + 1) % OPTION_MAX;
  } else if ((g->worms[PLAYER_1]->worms_action & ACTION_LEFT || g->worms[PLAYER_2]->worms_action & ACTION_LEFT) && wiiero_option[*selected_option_entry].option_on_left) {
    wiiero_option[*selected_option_entry].option_on_left(wiiero_option[*selected_option_entry].option_data);
  } else if ((g->worms[PLAYER_1]->worms_action & ACTION_RIGHT || g->worms[PLAYER_2]->worms_action & ACTION_RIGHT) && wiiero_option[*selected_option_entry].option_on_right) {
    wiiero_option[*selected_option_entry].option_on_right(wiiero_option[*selected_option_entry].option_data);
  } else if ((g->worms[PLAYER_1]->worms_action & ACTION_OK || g->worms[PLAYER_2]->worms_action & ACTION_OK) && wiiero_option[*selected_option_entry].option_action) {
    wiiero_option[*selected_option_entry].option_action(g);
  } else if (g->worms[PLAYER_1]->worms_action & ACTION_CANCEL || g->worms[PLAYER_2]->worms_action & ACTION_CANCEL) {
...
static __inline__ void wiiero_option(game_t *g)
{
  ...
  option_enty_t wiiero_option[OPTION_MAX] = {
      ...
      {OPTION_RESOLUTION, wiiero_label[WIIERO_LANG_OPT_MENU_RESOL], &(g->wiiero_opt_screen_resolution), option_format_resol_cb, option_laction_res_cb, option_raction_res_cb, option_change_res_cb},
  ...
  for (i = 0; i < OPTION_MAX; i++)
  {
    if (i != selected_option_entry)
      font_print_center(g->wiiero_cameras[PLAYER_2_GAME_ZONE_CAM], ((wiiero_option[i].option_formater) ? wiiero_option[i].option_formater(wiiero_option[i].label, wiiero_option[i].option_data) : wiiero_option[i].label), g->wiiero_cameras[PLAYER_2_GAME_ZONE_CAM]->w / 2 + 5, (g->wiiero_cameras[PLAYER_2_GAME_ZONE_CAM]->h - (OPTION_MAX * 10)) / 2 + (i * 10) + 5, FONT_STANDARD);
    else
      font_print_center(g->wiiero_cameras[PLAYER_2_GAME_ZONE_CAM], ((wiiero_option[i].option_formater) ? wiiero_option[i].option_formater(wiiero_option[i].label, wiiero_option[i].option_data) : wiiero_option[i].label), g->wiiero_cameras[PLAYER_2_GAME_ZONE_CAM]->w / 2 + 5, (g->wiiero_cameras[PLAYER_2_GAME_ZONE_CAM]->h - (OPTION_MAX * 10)) / 2 + (i * 10) + 5, FONT_SELECTED);
  }
  wiiero_option_actions(g, &selected_option_entry, OPTION_RETURN, OPTION_MAX, wiiero_option);
}
/*---------------------------------------------------------------------------*/
...
static __inline__ void wiiero_option_bis(game_t *g)
{
  ...
  {
    if (i != selected_option_entry)
      font_print_center(g->wiiero_cameras[PLAYER_2_GAME_ZONE_CAM], ((wiiero_option[i].option_formater) ? wiiero_option[i].option_formater(wiiero_option[i].label, wiiero_option[i].option_data) : wiiero_option[i].label), g->wiiero_cameras[PLAYER_2_GAME_ZONE_CAM]->w / 2 + 5, (g->wiiero_cameras[PLAYER_2_GAME_ZONE_CAM]->h - (OPTION_BIS_MAX * 10)) / 2 + (i * 10) + 5, FONT_STANDARD);
    else
      font_print_center(g->wiiero_cameras[PLAYER_2_GAME_ZONE_CAM], ((wiiero_option[i].option_formater) ? wiiero_option[i].option_formater(wiiero_option[i].label, wiiero_option[i].option_data) : wiiero_option[i].label), g->wiiero_cameras[PLAYER_2_GAME_ZONE_CAM]->w / 2 + 5, (g->wiiero_cameras[PLAYER_2_GAME_ZONE_CAM]->h - (OPTION_BIS_MAX * 10)) / 2 + (i * 10) + 5, FONT_SELECTED);
  }
  wiiero_option_actions(g, &selected_option_entry, OPTION_BIS_RETURN, OPTION_BIS_MAX, wiiero_option);
} /*---------------------------------------------------------------------------*/
...
static __inline__ void wiiero_set_message(game_t *g, char *message, fontsize_t font, game_status_t next_status, Uint8 alpha)
{
  ...
  /* SHOW MESSAGE */
  font_print_center(g->wiiero_cameras[FULL_SCREEN_CAM], message, g->wiiero_cameras[FULL_SCREEN_CAM]->w / 2, g->wiiero_cameras[FULL_SCREEN_CAM]->h / 3, font);
...
static __inline__ void wiiero_edit_name(game_t *g)
{
  ...
  camera_clean(g->wiiero_cameras[FULL_SCREEN_CAM]);
  font_print_center(g->wiiero_cameras[FULL_SCREEN_CAM], wiiero_label[WIIERO_LANG_PNAME_LABEL], g->wiiero_cameras[FULL_SCREEN_CAM]->w / 2, g->wiiero_cameras[FULL_SCREEN_CAM]->h / 4, FONT_STANDARD);
  ...
  for (i = 0; i < WIIERO_NAME_LEN; i++)
  {
    ...
    if (i == current_char)
    {
      font_putc_strict_pos(g->wiiero_cameras[FULL_SCREEN_CAM], caract, g->wiiero_cameras[FULL_SCREEN_CAM]->w / 2 + (i - WIIERO_NAME_LEN / 2) * 8, g->wiiero_cameras[FULL_SCREEN_CAM]->h / 3, FONT_SELECTED);
    }
    else
    {
      font_putc_strict_pos(g->wiiero_cameras[FULL_SCREEN_CAM], caract, g->wiiero_cameras[FULL_SCREEN_CAM]->w / 2 + (i - WIIERO_NAME_LEN / 2) * 8, g->wiiero_cameras[FULL_SCREEN_CAM]->h / 3, FONT_STANDARD);
    }
  }

  font_print_center(g->wiiero_cameras[FULL_SCREEN_CAM], wiiero_label[WIIERO_LANG_WSELECT_OK], 3 * g->wiiero_cameras[FULL_SCREEN_CAM]->w / 4, 3 * g->wiiero_cameras[FULL_SCREEN_CAM]->h / 4, FONT_STANDARD);
...
static __inline__ void wiiero_set_round_stats(game_t *g)
{
  ...
  font_print_center(g->wiiero_cameras[FULL_SCREEN_CAM], wiiero_label[WIIERO_LANG_WARNING_PRESSKEY], 3 * g->wiiero_cameras[FULL_SCREEN_CAM]->w / 4, 3 * g->wiiero_cameras[FULL_SCREEN_CAM]->h / 4, FONT_STANDARD);
...
static __inline__ void wiiero_how_to_play(game_t *g)
{
  ...
  font_print_center(g->wiiero_cameras[FULL_SCREEN_CAM], wiiero_label[WIIERO_LANG_CTRL_LABEL], g->wiiero_cameras[FULL_SCREEN_CAM]->w / 2, h, FONT_SELECTED);

  h += (font_get_height(FONT_SELECTED) + 4) + 8;
  for (lab_id = FIRE_CMD; lab_id < MAX_CMD; lab_id++)
    font_print_strict_pos(g->wiiero_cameras[FULL_SCREEN_CAM], wiiero_label[(WIIERO_LANG_CTRL_FIRST_ID - 1) + lab_id], w, h += (font_get_height(FONT_STANDARD) + 4), FONT_STANDARD);
...
static __inline__ void wiiero_about(game_t *g)
{
  ...
  camera_clean(g->wiiero_cameras[FULL_SCREEN_CAM]);
  camera_set_alpha(g->wiiero_cameras[FULL_SCREEN_CAM], 250);
  font_print_center(g->wiiero_cameras[FULL_SCREEN_CAM], WIIERO_VERSION, g->wiiero_cameras[FULL_SCREEN_CAM]->w / 2, h, FONT_SELECTED);

  h += (font_get_height(FONT_SELECTED) + 4);
  for (lab_id = 0; lab_id < ABOUT_LINES; lab_id++)
    font_print_center(g->wiiero_cameras[FULL_SCREEN_CAM], about[lab_id], g->wiiero_cameras[FULL_SCREEN_CAM]->w / 2, h += (font_get_height(FONT_STANDARD) + 4), FONT_STANDARD);
